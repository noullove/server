 ...
 * reference : Oracle Multithreaded Programming Guide
 */
...
static int create_worker(thr_pool_t* pool)
{
	...
	error = pthread_create(&t, &pool->pool_attr, worker_thread, pool);
...
thr_pool_t* thr_pool_create(int min_threads, int max_threads, int linger, pthread_attr_t* attr)
{
	thr_pool_t* pool;
...
{
	(void)pthread_mutex_lock(&pool->pool_mutex);
	pthread_cleanup_push(cleanup_unlock_mutex, &pool->pool_mutex) ;
